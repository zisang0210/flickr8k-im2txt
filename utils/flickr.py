...
class FLICKR:
    def __init__(self, caption_file=None, image_file=None):
        """
        ...
        if not (caption_file == None or image_file == None):
            print('loading annotations into memory...')
            tic = time.time()
            dataset = self.load_dataset(caption_file,image_file)
            print('Done (t=%0.2fs)'%(time.time()- tic))
            self.dataset = dataset
            self.process_dataset()
            self.createIndex()
    
    def load_dataset(self, caption_file,image_file):
        """
        Returns:
            A dict containing image filename and corresponding captions info.
            {
            "annotations":[{"id":1,"image_id":1,"caption":"a cat is sat on the mat"},{...},...],
            "images":[{"id": 398331,"file_name": "COCO_train2014_000000398331.jpg"},{...},...]
            }
        """
        captions = self.load_captions(caption_file)
        train_imgs = self.load_image_filename(image_file)
        ...
        for img in train_imgs:
            img_id = img_id + 1
            imgs.append({"id":img_id,"file_name":img})
            for cap in captions[img]:
                ann_id = ann_id + 1            
                anns.append({"id":ann_id,"image_id":img_id,"caption":cap})

        return {"annotations":anns,"images":imgs}


    def load_image_filename(self, filepath):
    ...
    def load_captions(self, captions_file):
        ...
        Returns:
            A map from image filename to captions.
        """
