...
class NN(object):
    def __init__(self, config):
        self.config = config
        self.is_train = config.phase == 'train' and config.joint_train
    ...
    def att_dense(self,
              ...
              use_bias = True,
              name = None):
        """ Fully-connected layer, similar to self.dense. However the function is for bias 
        attention mechanism only. We want to train this part only whiling keeping other 
        weights fixed. Then the function is replace by self.dense to do joint training"""
        if activation is not None:
            activity_regularizer = self.fc_activity_regularizer
        else:
            activity_regularizer = None
